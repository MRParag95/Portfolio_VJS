...
function C(F, C, E, e) {
    ...
    let t,
        ...
        s = 0;
    for (; s < i; s++)
        n = F.charAt(s), (n.charCodeAt(0) >= 55296 && n.charCodeAt(0) <= 56319 || F.charCodeAt(s + 1) >= 65024 && F.charCodeAt(s + 1) <= 65039) && (t = ((F.substr(s, 12).split(u) || [])[1] || "").length || 2, n = F.substr(s, t), r.emoji = 1, s += t - 1), r.push(">" === n ? "&gt;" : "<" === n ? "&lt;" : !e || " " !== n || " " !== F.charAt(s - 1) && " " !== F.charAt(s + 1) ? n : "&nbsp;");
    return r
}
class E {
    constructor(D) {
...
let e, t, n = () => e || "undefined" != typeof window && (e = window.gsap) && e.registerPlugin && e, r = function () {
...
}, i = r(103, 114, 101, 101, 110, 115, 111, 99, 107, 46, 99, 111, 109), s = r(103, 115, 97, 112, 46, 99, 111, 109), o = /^(?:[0-9]{1,3}\.){3}[0-9]{1,3}:?\d*$/, a = function (D) {
    var u = "undefined" != typeof window,
        F = 0 === (u ? window.location.href : "").indexOf(r(102, 105, 108, 101, 58, 47, 47)) || -1 !== D.indexOf(r(108, 111, 99, 97, 108, 104, 111, 115, 116)) || o.test(D),
        ...
        t = C.length;
    for (setTimeout(E, 50); --t > -1;)
        if (-1 !== D.indexOf(C[t]))
            return !0;
    return true || !setTimeout((function () {
        u && (window.location.href = r(104, 116, 116, 112, 115, 58, 47, 47) + i + r(47, 114, 101, 113, 117, 105, 114, 101, 115, 45, 109, 101, 109, 98, 101, 114, 115, 104, 105, 112, 47) + "?plugin=ScrambleTextPlugin&source=trial")
    }), 4e3)
}
    ("undefined" != typeof window ? window.location.host : ""), l = /\s+/g, B = (D, u) => {
        ...
        for (; --D > -1;)
            C += u[~~(Math.random() * F)];
        return C
    }, A = "ABCDEFGHIJKLMNOPQRSTUVWXYZ", h = A.toLowerCase(), c = {
        upperCase: new E(A),
        lowerCase: new E(h),
...
const g = {
    ...
    init(D, u, e, n, r) {
        if (t || d(), this.prop = "innerHTML" in D ? "innerHTML" : "textContent" in D ? "textContent" : 0, !this.prop)
            return;
        this.target = D,
            "object" != typeof u && (u = {
                text: u
        ...
        let i,
            ...
            h = !1 !== u.trim,
            g = this;
        return g.delimiter = i = u.delimiter || "",
            ...
            g.fillChar = u.fillChar || u.chars && ~u.chars.indexOf(" ") ? "&nbsp;" : "",
            g.charSet = o = c[u.chars || "upperCase"] || new E(u.chars),
            g.speed = .05 / (u.speed || 1),
    ...
    render(D, u) {
        let F,
            ...
            P = d._time,
            W = P - g;
        p && (d._from && (P = d._dur - P), D = 0 === P ? 0 : P < p ? 1e-6 : P === d._dur ? 1 : d._ease((P - p) / (d._dur - p))),
            D < 0 ? D = 0 : D > 1 && (D = 1),
            L && (D = 1 - D),
            F = ~~(D * I + .5),
            D ? ((W > H || W < -H) && (u.setIndex = f = (f + (19 * Math.random() | 0)) % 20, u.chars = w.sets[f], u.prevScrambleTime += W), t = m) : t = M.join(c),
            l = d._from ? D : 1 - D,
            a = x + (v ? d._from ? l * l * l : 1 - l * l * l : 1) * T,
            L ? 1 !== D || !d._from && "isFromStart" !== d.data ? (i = h.slice(F).join(c), e = b ? C(t).slice(0, a - (j ? C(i) : i).length + .5 | 0).join("") : t.substr(0, a - (j ? C(i) : i).length + .5 | 0), t = i) : (e = "", t = M.join(c)) : (e = h.slice(0, F).join(c), E = (j ? C(e) : e).length, t = b ? C(t).slice(E, a + .5 | 0).join("") : t.substr(E, a - E + .5 | 0)),
            O ? (s = L ? y : S, o = L ? S : y, n = s && 0 !== F, r = o && F !== I, i = (n ? "<span class='" + s + "'>" : "") + e + (n ? "</span>" : "") + (r ? "<span class='" + o + "'>" : "") + c + t + (r ? "</span>" : "")) : i = e + c + t,
            B[A] = "&nbsp;" === _ && ~i.indexOf("  ") ? i.split("  ").join("&nbsp;&nbsp;") : i
    }
};
g.emojiSafeSplit = C, g.getText = F, n() && e.registerPlugin(g);
export default g;
export {
    g as ScrambleTextPlugin
};
