...
let e = /[achlmqstvz]|(-?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/gi, t = /[\+\-]?\d*\.?\d+e[\+\-]?\d+/gi, n = Math.PI / 180, s = Math.sin, i = Math.cos, o = Math.abs, r = Math.sqrt, a = e => Math.round(1e5 * e) / 1e5 || 0;
function l(e, t, a, l, h, c, g, u, f) {
    ...
    let z,
        ...
        B = x * l;
    for (z = 0; z < H; z++)
        C = i(h = R + z * Z), E = s(h), _ = i(h += Z), j = s(h), Q.push(C - U * E, E + U * C, _ + U * j, j - U * _, _, j);
...
let h, c, g = () => h || "undefined" != typeof window && (h = window.gsap) && h.registerPlugin && h, u = () => {
    h = g(),
        h ? (h.registerEase("_CE", E.create), c = 1) : console.warn("Please gsap.registerPlugin(CustomEase)")
}, f = e => ~~(1e3 * e + (e < 0 ? - .5 : .5)) / 1e3, d = function () {
    return String.fromCharCode.apply(null, arguments)
}, x = d(103, 114, 101, 101, 110, 115, 111, 99, 107, 46, 99, 111, 109), p = d(103, 115, 97, 112, 46, 99, 111, 109), y = /^(?:[0-9]{1,3}\.){3}[0-9]{1,3}:?\d*$/, m = function (e) {
    var t = "undefined" != typeof window,
        n = 0 === (t ? window.location.href : "").indexOf(d(102, 105, 108, 101, 58, 47, 47)) || -1 !== e.indexOf(d(108, 111, 99, 97, 108, 104, 111, 115, 116)) || y.test(e),
    ...
    ("undefined" != typeof window ? window.location.host : ""), w = /[-+=\.]*\d+[\.e\-\+]*\d*[e\-\+]*\d*/gi, M = /[cLlsSaAhHvVtTqQ]/g, C = function (e, t, n, s, i, o, r, a, l, h, c) {
        ...
        return h || (h = [{
            ...
            (L + P) * (L + P) > l * (S * S + N * N) && (g = h.length, C(e, t, u, f, m, w, b, v, l, h, c), C(b, v, M, E, p, y, r, a, l, h, c + 1 + (h.length - g))),
            h
    };
class E {
    constructor(e, t, n) {
    ...
    setData(n, s) {
        ...
        let i,
            ...
            b = s.precision || 1,
            v = b <= 1;
        if (this.data = n, (M.test(n) || ~n.indexOf("M") && n.indexOf("C") < 0) && (p = function (n) {
            let s,
                i,
                r,
                a,
            ...
            if (!n || !isNaN(M[0]) || isNaN(M[1]))
                return console.log(N), C;
            for (s = 0; s < v; s++)
                if (y = h, isNaN(M[s]) ? (h = M[s].toUpperCase(), c = h !== M[s]) : s--, r = +M[s + 1], a = +M[s + 2], c && (r += E, a += b), s || (u = r, f = a), "M" === h)
                    g && (g.length < 8 ? C.length -= 1 : S += g.length), E = u = r, b = f = a, g = [r, a], C.push(g), s += 2, h = "L";
                else if ("C" === h)
                    g || (g = [0, 0]), c || (E = b = 0), g.push(r, a, E + 1 * M[s + 3], b + 1 * M[s + 4], E += 1 * M[s + 5], b += 1 * M[s + 6]), s += 6;
                else if ("S" === h)
                    d = E, x = b, "C" !== y && "S" !== y || (d += E - g[g.length - 4], x += b - g[g.length - 3]), c || (E = b = 0), g.push(d, x, r, a, E += 1 * M[s + 3], b += 1 * M[s + 4]), s += 4;
                else if ("Q" === h)
                    d = E + 2 / 3 * (r - E), x = b + 2 / 3 * (a - b), c || (E = b = 0), E += 1 * M[s + 3], b += 1 * M[s + 4], g.push(d, x, E + 2 / 3 * (r - E), b + 2 / 3 * (a - b), E, b), s += 4;
                else if ("T" === h)
                    d = E - g[g.length - 4], x = b - g[g.length - 3], g.push(E + d, b + x, r + 2 / 3 * (E + 1.5 * d - r), a + 2 / 3 * (b + 1.5 * x - a), E = r, b = a), s += 2;
                else if ("H" === h)
                    L(E, b, E = r, b), s += 1;
                else if ("V" === h)
                    L(E, b, E, b = r + (c ? b - E : 0)), s += 1;
                else if ("L" === h || "Z" === h)
                    "Z" === h && (r = u, a = f, g.closed = !0), ("L" === h || o(E - r) > .5 || o(b - a) > .5) && (L(E, b, r, a), "L" === h && (s += 2)), E = r, b = a;
                else if ("A" === h) {
                    if (m = M[s + 4], w = M[s + 5], d = M[s + 6], x = M[s + 7], i = 7, m.length > 1 && (m.length < 3 ? (x = d, d = w, i--) : (x = w, d = m.substr(2), i -= 2), w = m.charAt(1), m = m.charAt(0)), p = l(E, b, +M[s + 1], +M[s + 2], +M[s + 3], +m, +w, (c ? E : 0) + 1 * d, (c ? b : 0) + 1 * x), s += i, p)
            ...
            (n)[0]), i = p.length, 4 === i) p.unshift(0, 0), p.push(1, 1), i = 8;
        else if ((i - 2) % 6)
            throw "Invalid CustomEase";
        for (0 == +p[0] && 1 == +p[i - 2] || ((e, t, n) => {
            ...
            for (l = l ? 1 / l : -a, s = 0; s < r; s += 2)
                e[s] = (+e[s] + i) * a, e[s + 1] = (+e[s + 1] + o) * l
        })(p, s.height, s.originY), this.segment = p, c = 2; c < i; c += 6)r = {
            x: +p[c - 2],
            y: +p[c - 1]
        },
            a = {
        ...
        for (i = m.length, c = 0; c < i; c++)
            f = m[c], d = m[c - 1] || f, (f.x > d.x || d.y !== f.y && d.x === f.x || f === d) && f.x <= 1 ? (d.cx = f.x - d.x, d.cy = f.y - d.y, d.n = f, d.nx = f.x, v && c > 1 && Math.abs(d.cy / d.cx - m[c - 2].cy / m[c - 2].cx) > 2 && (v = 0), d.cx < y && (d.cx ? y = d.cx : (d.cx = .001, c === i - 1 && (d.x -= .001, y = Math.min(y, .001), v = 0)))) : (m.splice(c--, 1), i--);
        if (i = 1 / y + 1 | 0, g = 1 / i, u = 0, f = m[0], v) {
            for (c = 0; c < i; c++)
                x = c * g, f.nx < x && (f = m[++u]), r = f.y + (x - f.x) / f.cx * f.cy, E[c] = {
                    x: x,
                    cx: g,
                    y: r,
                    cy: 0,
    ...
    static getSVGData(e, t) {
        ...
        if (t.invert && (p = -p, m = 0), "string" == typeof e && (e = h.parseEase(e)), e.custom && (e = e.custom), e instanceof E)
            n = function (e) {
                ...
                for (n = 0; n < r; n++) {
                    for (i = e[n], o += "M" + a(i[0]) + "," + a(i[1]) + " C", t = i.length, s = 2; s < t; s++)
                        o += a(i[s++]) + "," + a(i[s++]) + " " + a(i[s++]) + "," + a(i[s++]) + " " + a(i[s++]) + "," + a(i[s]) + " ";
                    i.closed && (o += "z")
                }
                return o
            }
            ...
            for (n = [y, m], c = Math.max(5, 200 * (t.precision || 1)), o = 1 / c, c += 2, g = 5 / c, u = f(y + o * x), d = f(m + e(o) * -p), s = (d - m) / (u - y), i = 2; i < c; i++)
                r = f(y + i * o * x), l = f(m + e(i * o) * -p), (Math.abs((l - d) / (r - u) - s) > g || i === c - 1) && (n.push(u, d), s = (l - d) / (r - u)), u = r, d = l;
            n = "M" + n.join(",")
        }
        return w && w.setAttribute("d", n),
            n
    }
}
